...
if (is_object($cookie)) {
	...
	if (Validate::isLoadedObject($employee) && $employee->checkPassword((int)$cookie->id_employee, $cookie->passwd) && (!isset($cookie->remote_addr) || $cookie->remote_addr == ip2long(Tools::getRemoteAddr()) || !Configuration::get('PS_COOKIE_CHECKIP')))
		...
		$_GET['login'] = 'Logged with: <b>'.$employee->lastname.' '.$employee->firstname.'</b>';
...
function et($tag){
    ...
    $fr1['Decompress'] = 'Décompresser';

    $fr1['ResolveIDs'] = 'Résoudre les IDs';

    $fr1['Move'] = 'Déplacer';

    $fr1['Copy'] = 'Copier';

    $fr1['ServerInfo'] = 'Infos serveur';

    $fr1['CreateDir'] = 'Créer un répertoire';

    $fr1['CreateArq'] = 'Créer un fichier';

    $fr1['ExecCmd'] = 'Exécuter une Commande';
    ...
    $fr1['SelDir'] = 'Sélectionnez le répertoire de destination dans le panneau gauche';
    ...
    $fr1['IOError'] = 'Erreur I/O';

    $fr1['FileMan'] = 'Gestionnaire PHP';
    ...
    $fr2['SendingForm'] = 'Envoi de fichier, veuillez patienter';
...
function dir_list_form() {
    ...
    if ($opdir = @opendir($current_dir)) {
        ...
        $out .= "
			...
			<span style=\"float:right;\">".$_GET['login']."</span>
